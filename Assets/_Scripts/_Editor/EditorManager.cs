...
public class EditorManager : MonoBehaviour
{
    ...
    public bool bluePowerInstantiated = false;
        ...
        if (editorMode && !instantiated)
        {
            switch (itemId)
            {
                case 1:
                    ...
                    if (!bluePowerInstantiated)
                    {
                        SpikeBall spike1 = new SpikeBall(item, new BlueMat());
                        subject.AddObserver(spike1);
                        bluePowerInstantiated = true;
                    }
                    else
                    {
                        SpikeBall spike1 = new SpikeBall(item, new YellowMat());
                        subject.AddObserver(spike1);
                    }
                    break;
                case 2:
